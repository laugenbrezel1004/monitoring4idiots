<script lang="ts">
	...
	onMount(() => {
		const legend = document.getElementsByClassName('lc-legend-container').item(0);
		if(!legend) return;
		legend.setAttribute('style', 'margin-left: 4.5rem');
	});
...
<Card.Root class="@container/card">
	...
	<Card.Content class="px-2 pt-4 sm:px-6 sm:pt-6">
		<Chart.Container config={chartConfig} class="aspect-auto h-[250px] w-full flex items-center">
			<AreaChart
				legend
				data={filteredData}
				x="date"
